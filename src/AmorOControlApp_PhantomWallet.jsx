...
function Solana() {
  ...
  async function signProofOfCare() {
    ...
    try {
      ...
      setStatus("Proof of Care firmado. Esto verifica propiedad de wallet sin guardar información sensible.");
    } catch (error) {
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_.8fr]">
        <Card>
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50/90 p-4 text-sm leading-6 text-slate-700 shadow-inner">
            Esta integración usa Phantom como capa de identidad pseudónima. La wallet sirve para demostrar propiedad de una dirección y firmar un Proof of Care. La evidencia, historias, ubicación, contactos, capturas y niveles de riesgo no se guardan en blockchain.
          </div>
          ...
          {signature && (
            <div className="mt-6 rounded-2xl border border-teal-200 bg-teal-50/90 p-4 text-sm text-teal-950 shadow-inner">
              <p className="font-black">Firma generada por Phantom</p>
              <p className="mt-2 break-all font-mono text-xs leading-6">{signature}</p>
              <Button variant="secondary" className="mt-3" onClick={() => copyToClipboard(signature, "Firma")}>Copiar firma</Button>
            </div>
          )}
        </Card>
